...
{
    public class UserRoles : DBEntity, IDBEntity, IDisposable, IEqualityComparer
    {
        ...
        public UserRoles() : base() { SetupDBEntity(); }
        public UserRoles(DBConnector p_Connection) : base(p_Connection) { SetupDBEntity(); }
        public UserRoles(string p_ConnectionString) : base(p_ConnectionString) { SetupDBEntity(); }

        private void SetupDBEntity()
        {
            string l_Query = string.Empty;

            if (string.IsNullOrEmpty(UserRoles.TableName)) UserRoles.TableName = "UserRoles";
            if (string.IsNullOrEmpty(UserRoles.PrimaryKeyName)) UserRoles.PrimaryKeyName = "Id";
            if (string.IsNullOrEmpty(UserRoles.EndingPropertyName)) UserRoles.EndingPropertyName = "CreatedBy";
            if (UserRoles.DBProperties == null) UserRoles.DBProperties = new List<PropertyInfo>(this.GetType().GetProperties());
            if (string.IsNullOrEmpty(UserRoles.InsertQueryStart))
                UserRoles.InsertQueryStart = PrepareQueries(this, UserRoles.TableName, UserRoles.EndingPropertyName, ref l_Query, UserRoles.DBProperties);
        }
        ...
        public bool GetList(string p_Criteria, string p_Fields, ref DataTable p_Data, string p_OrderBy = "")
        {
            string l_Query = string.IsNullOrEmpty(p_Fields)
                ? "SELECT * FROM [" + UserRoles.TableName + "]"
                : "SELECT " + p_Fields + " FROM [" + UserRoles.TableName + "]";
        ...
        public int GetMax()
        {
            ...
            if (!l_Common.GetList("SELECT MAX(CONVERT(INT, ISNULL(" + UserRoles.PrimaryKeyName + ", '0'))) FROM " + UserRoles.TableName, ref l_Data))
                return l_MaxNo;
        ...
        public Result SaveNew()
        {
            ...
            try
            {
                ...
                this.Id = this.GetMax();
                string l_Query = this.PrepareInsertQuery(this, UserRoles.InsertQueryStart, UserRoles.EndingPropertyName, UserRoles.DBProperties);
        ...
        public Result DeleteByUserId(int userId)
        {
            ...
            {
                l_Trans = this.Connection.BeginTransaction();
                string l_Query = $"DELETE FROM [{UserRoles.TableName}] WHERE UserId = {userId}";
                l_Process = this.Connection.Execute(l_Query);
        ...
        public Result Delete()
        {
            ...
            {
                l_Trans = this.Connection.BeginTransaction();
                string l_Query = this.PrepareDeleteQuery(this, UserRoles.TableName, UserRoles.PrimaryKeyName);
                l_Process = this.Connection.Execute(l_Query);
        ...
        public new bool Equals(object x, object y) { return ((UserRoles)x).Id == ((UserRoles)y).Id; }
