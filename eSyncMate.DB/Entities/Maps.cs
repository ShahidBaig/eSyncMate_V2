...
namespace eSyncMate.DB.Entities
{
    public class Maps : DBEntity, IDBEntity, IDisposable, IEqualityComparer
    {
        ...
        {
            totalCount = 0;
            string l_CountQuery = "SELECT COUNT(*) FROM [" + Maps.ViewName + "]";
            ...
            string l_Query = string.IsNullOrEmpty(p_Fields)
                ? "SELECT * FROM [" + Maps.ViewName + "]"
                : "SELECT " + p_Fields + " FROM [" + Maps.ViewName + "]";
        ...
        public Result Modify()
        {
            ...
            try
            {
                Maps.DBProperties = this.GetType().GetProperties()
                  .Where(prop => prop.Name != "CreatedBy" && prop.Name != "CreatedDate")
                  .ToList();

                Maps.EndingPropertyName = "ModifiedBy";
