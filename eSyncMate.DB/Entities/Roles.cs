...
{
    public class Roles : DBEntity, IDBEntity, IDisposable, IEqualityComparer
    {
        ...
        public Roles() : base() { SetupDBEntity(); }
        public Roles(DBConnector p_Connection) : base(p_Connection) { SetupDBEntity(); }
        public Roles(string p_ConnectionString) : base(p_ConnectionString) { SetupDBEntity(); }

        private void SetupDBEntity()
        {
            string l_Query = string.Empty;

            if (string.IsNullOrEmpty(Roles.TableName)) Roles.TableName = "Roles";
            if (string.IsNullOrEmpty(Roles.ViewName)) Roles.ViewName = "Roles";
            if (string.IsNullOrEmpty(Roles.PrimaryKeyName)) Roles.PrimaryKeyName = "Id";
            if (string.IsNullOrEmpty(Roles.EndingPropertyName)) Roles.EndingPropertyName = "CreatedBy";
            if (Roles.DBProperties == null) Roles.DBProperties = new List<PropertyInfo>(this.GetType().GetProperties());
            if (string.IsNullOrEmpty(Roles.InsertQueryStart))
                Roles.InsertQueryStart = PrepareQueries(this, Roles.TableName, Roles.EndingPropertyName, ref l_Query, Roles.DBProperties);
        }
        ...
        public bool GetList(string p_Criteria, string p_Fields, ref DataTable p_Data, string p_OrderBy = "")
        {
            string l_Query = string.IsNullOrEmpty(p_Fields)
                ? "SELECT * FROM [" + Roles.TableName + "]"
                : "SELECT " + p_Fields + " FROM [" + Roles.TableName + "]";
        ...
        {
            SetProperty(Roles.PrimaryKeyName, p_PrimaryKey);
            return GetObjectFromQuery(PrepareGetObjectQuery(this, Roles.TableName, Roles.PrimaryKeyName));
        }

        public Result GetObjectFromQuery(string p_Query, bool isOnlyObject = false)
        {
            DataTable l_Data = new DataTable();
            if (!Connection.GetData(p_Query, ref l_Data)) return Result.GetNoRecordResult();
        ...
        public int GetMax()
        {
            ...
            if (!l_Common.GetList("SELECT MAX(CONVERT(INT, ISNULL(" + Roles.PrimaryKeyName + ", '0'))) FROM " + Roles.TableName, ref l_Data))
                return l_MaxNo;
        ...
        public Result SaveNew()
        {
            ...
            try
            {
                ...
                this.Id = this.GetMax();
                string l_Query = this.PrepareInsertQuery(this, Roles.InsertQueryStart, Roles.EndingPropertyName, Roles.DBProperties);
        ...
        public Result Modify()
        {
            ...
            {
                l_Trans = this.Connection.BeginTransaction();
                string l_Query = this.PrepareUpdateQuery(this, Roles.TableName, Roles.PrimaryKeyName, Roles.EndingPropertyName, Roles.DBProperties);
                l_Process = this.Connection.Execute(l_Query);
        ...
        public Result Delete()
        {
            ...
            {
                l_Trans = this.Connection.BeginTransaction();
                string l_Query = this.PrepareDeleteQuery(this, Roles.TableName, Roles.PrimaryKeyName);
                l_Process = this.Connection.Execute(l_Query);
        ...
        public new bool Equals(object x, object y) { return ((Roles)x).Id == ((Roles)y).Id; }
