...
{
    public class Modules : DBEntity, IDBEntity, IDisposable, IEqualityComparer
    {
        ...
        public Modules() : base() { SetupDBEntity(); }
        public Modules(DBConnector p_Connection) : base(p_Connection) { SetupDBEntity(); }
        public Modules(string p_ConnectionString) : base(p_ConnectionString) { SetupDBEntity(); }

        private void SetupDBEntity()
        {
            string l_Query = string.Empty;

            if (string.IsNullOrEmpty(Modules.TableName)) Modules.TableName = "Modules";
            if (string.IsNullOrEmpty(Modules.ViewName)) Modules.ViewName = "Modules";
            if (string.IsNullOrEmpty(Modules.PrimaryKeyName)) Modules.PrimaryKeyName = "Id";
            if (string.IsNullOrEmpty(Modules.EndingPropertyName)) Modules.EndingPropertyName = "CreatedBy";
            if (Modules.DBProperties == null) Modules.DBProperties = new List<PropertyInfo>(this.GetType().GetProperties());
            if (string.IsNullOrEmpty(Modules.InsertQueryStart))
                Modules.InsertQueryStart = PrepareQueries(this, Modules.TableName, Modules.EndingPropertyName, ref l_Query, Modules.DBProperties);
        }
        ...
        public bool GetList(string p_Criteria, string p_Fields, ref DataTable p_Data, string p_OrderBy = "")
        {
            string l_Query = string.IsNullOrEmpty(p_Fields)
                ? "SELECT * FROM [" + Modules.TableName + "]"
                : "SELECT " + p_Fields + " FROM [" + Modules.TableName + "]";
        ...
        public int GetMax()
        {
            ...
            if (!l_Common.GetList("SELECT MAX(CONVERT(INT, ISNULL(" + Modules.PrimaryKeyName + ", '0'))) FROM " + Modules.TableName, ref l_Data))
                return l_MaxNo;
        ...
        public Result SaveNew()
        {
            ...
            try
            {
                ...
                this.Id = this.GetMax();
                string l_Query = this.PrepareInsertQuery(this, Modules.InsertQueryStart, Modules.EndingPropertyName, Modules.DBProperties);
        ...
        public Result Modify()
        {
            ...
            {
                l_Trans = this.Connection.BeginTransaction();
                string l_Query = this.PrepareUpdateQuery(this, Modules.TableName, Modules.PrimaryKeyName, Modules.EndingPropertyName, Modules.DBProperties);
                l_Process = this.Connection.Execute(l_Query);
        ...
        public Result Delete()
        {
            ...
            {
                l_Trans = this.Connection.BeginTransaction();
                string l_Query = this.PrepareDeleteQuery(this, Modules.TableName, Modules.PrimaryKeyName);
                l_Process = this.Connection.Execute(l_Query);
        ...
        public new bool Equals(object x, object y) { return ((Modules)x).Id == ((Modules)y).Id; }
