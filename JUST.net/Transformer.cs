...
namespace JUST
{
    ...
    public abstract class Transformer<T> : Transformer where T : ISelectableToken
    {
        ...
        public static object sumproductatpath(JArray parsedArray, string qtyPath, string pricePath, JUSTContext context)
        {
            decimal result = 0m;

            if (parsedArray != null)
            {
                foreach (JToken item in parsedArray.Children())
                {
                    var selector = context.Resolve<T>(item); // same pattern as your sumatpath
                    JToken qtyTok = selector.Select(qtyPath);
                    JToken priceTok = selector.Select(pricePath);

                    decimal q = ToDecimalSafe(qtyTok);
                    decimal p = ToDecimalSafe(priceTok);

                    result += q * p;        // item-wise (qty * price)
                }
            }

            return TypedNumber(result);     // same helper you use in sum()
        }

        private static decimal ToDecimalSafe(JToken t)
        {
            ...
            if (t.Type == JTokenType.String &&
                decimal.TryParse(t.Value<string>(), NumberStyles.Any, CultureInfo.InvariantCulture, out var d))
                return d;
