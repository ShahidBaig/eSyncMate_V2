...
@Component({
  selector: 'alert-config-help-dialog',
  templateUrl: './alert-config-help-dialog.component.html',
  styleUrls: ['./alert-config-help-dialog.component.scss'],
  standalone: true,
  imports: [MatDialogModule, MatIconModule, MatButtonModule, MatDividerModule, CommonModule, MatTooltipModule],
})
export class AlertConfigHelpDialogComponent {
  @ViewChild('helpContent', { static: false }) helpContent!: ElementRef;
  constructor(public dialogRef: MatDialogRef<AlertConfigHelpDialogComponent>) {}
...
<head>
  <meta charset="utf-8">
  <title>Alert Configuration — Help Guide</title>
...
<body>
  <div class="help-content">${content}</div>
    ...
    link.download = 'Alert-Configuration-Help-Guide.html';
    link.click();
