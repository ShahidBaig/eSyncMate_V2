...
{
    public static class CarrierLoadTender214X6Route
    {
        public static void Execute(IConfiguration config, ILogger logger, Routes route)
        {
            ...
            try
            {
                ...
                {
                    route.SaveLog(LogTypeEnum.Exception, $"Required maps for 214 X6 processing is missing.", string.Empty, userNo);
                    return;
                }
                ...
                List<CarrierLoadTender> l_Carriers = l_Carrier.GetX6AcknowledgedCLTs(route.DestinationPartyId);
                foreach(CarrierLoadTender carrier in l_Carriers)
                {
                    try
                    {
                        ...
                        if (carrier.LastTrackStatus == carrier.TrackStatus)
                        {
                            carrier.TrackStatus = "X6";
                        }
                        ...
                        jsonTransformation = jsonTransformation.Replace("@ConsigneeName@", "No Name");
                        ...
                        if (l_OData.SaveNew().IsSuccess)
                        {
                            ...
                            CarrierLoadTender214X6Route.AddEDISegments(edimap, t, l_214.StartNodes, "L_N1");

                            CarrierLoadTender214X6Route.AddEDISegments(edimap, t, l_214.Packs.Data, "L_LX|L_AT7");
                            ...
                            if (l_Outbound.SaveNew().IsSuccess)
                            {
                                ...
                                if (l_OData.SaveNew().IsSuccess)
                                {
                                    ...
                                    if (l_Outbound.Modify().IsSuccess)
                                    {
                                        ...
                                        //if (carrier.TrackStatus != "D1")
                                        //{
                                        //    carrier.UpdateTrackStatus(carrier.Id, "X6");
                                        //}

                                        //carrier.UpdateTrackStatus(carrier.Id, carrier.TrackStatus);
                            ...
                            if (l_ParentNodeNames.Length > 1)
                            {
                                ...
                                if (l_SegmentDef == null)
                                {
                                    ...
                                    while (l_index < l_ParentNodeNames.Length)
                                    {
                                        try
                                        {
                                            l_SegmentDef = (MapSegment)(CarrierLoadTender214X6Route.FindMapLoopDef(l_MapLoop, l_ParentNodeNames[l_index])).Content.First(s => s.Name == l_Node.Name);
                                            break;
                                        }
                                        catch (InvalidOperationException)
                                        {
