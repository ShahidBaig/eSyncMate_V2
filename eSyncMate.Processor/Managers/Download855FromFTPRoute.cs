...
{
    public static class Download855FromFTPRoute
    {
        public static void Execute(IConfiguration config, ILogger logger, Routes route)
        {
            int userNo = 1;
            DataTable l_PurchaseOrderDt = new DataTable();
                                ...
                                foreach (EdiGroup g in i.Groups)
                                {
                                    ...
                                    foreach (EdiTrans t in g.Transactions)
                                    {
                                        ...
                                        try
                                        {
                                            ...
                                            Download855ResponseModel? received = JsonConvert.DeserializeObject<Download855ResponseModel>(jsonTransformation);
                                        ...
                                        finally
                                        {
                                            if(l_PurchaseOrders != null)
                                            {
                                                string edi_997 = CarrierLoadTenderManager.Surgimac855Generate997(l_EDIInfo, t, "855");

                                                if (!string.IsNullOrEmpty(edi_997))
                                                {
                                                    route.SaveData("ACKNOWLEDGED", 0, edi_997, userNo);
                                                    var method = "/outbound/997";
                                                    FtpConnector.Execute(l_SourceConnector.Host, l_SourceConnector.ConsumerKey, l_SourceConnector.ConsumerSecret, method, false, $"{Path.GetFileNameWithoutExtension(file.Key)}-997", edi_997).GetAwaiter().GetResult();
                                                }
                                            }
