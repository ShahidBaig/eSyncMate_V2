...
{
    public class GenerateEDI810ForRepaintRoute
    {
        ...
        public static void Execute(IConfiguration config, ILogger logger, Routes route)
        {
            ...
            try
            {
                ...
                {
                    route.SaveLog(LogTypeEnum.Error, $"Required map for 810 processing is missing.", string.Empty, userNo);
                    return;
                }
                ...
                if (l_SourceConnector.ConnectivityType == ConnectorTypesEnum.SqlServer.ToString())
                {
                    ...
                    l_SourceConnector.Command = l_SourceConnector.Command.Replace("@ORDERSTATUS@", "SHIPPED");
                ...
                if (l_DestinationConnector.ConnectivityType == ConnectorTypesEnum.Rest.ToString() && l_dataTable.Rows.Count > 0)
                {
                    ...
                    foreach (DataRow item in l_dataTable.Rows)
                    {
                        ...
                        OrderStatus855 l_810 = JsonConvert.DeserializeObject<OrderStatus855>(jsonTransformation);

                        Maps_5010.M_810 l_map = new Maps_5010.M_810();
                        EdiTrans t = new EdiTrans(l_map);
                        int l_HLIndex = 1, l_HL_PIndex = 0;

                        GenerateEDI810ForRepaintRoute.Add810Segments(l_map, t, l_810.StartNodes, "L_N1", ref l_HLIndex, l_HL_PIndex);

                        foreach (LoopItem l_Item in l_810.Items)
                        {
                            GenerateEDI810ForRepaintRoute.Add810Segments(l_map, t, l_Item.Data, "L_IT1", ref l_HLIndex, l_HL_PIndex);
                        }

                        GenerateEDI810ForRepaintRoute.Add810Segments(l_map, t, l_810.EndNodes, "L_ISS", ref l_HLIndex, l_HL_PIndex);
                        ...
                        if (l_Outbound.SaveNew().IsSuccess)
                        {
                            ...
                            l_EDIInfo.ISAReceiverQual, l_EDIInfo.ISAReceiverId, l_EDIInfo.ISASenderQual, l_Order.Customer.ISA810ReceiverId, l_EDIInfo.GSReceiverId, l_Order.Customer.ISA810ReceiverId,
                            ...
                            l_OData.DeleteWithType(l_Order.Order.Id, "810-eSyncMate");
                            ...
                            if (l_OData.SaveNew().IsSuccess)
                            {
                                if (!string.IsNullOrEmpty(l_OData.Data))
                                {
                                    ...
                                    SftpConnector.Execute(l_DestinationConnector, false, $"{l_OData.OrderNumber}-810", l_OData.Data).GetAwaiter().GetResult();

                                    DBConnector connection = new DBConnector(l_SourceConnector.ConnectionString);
                                    string Command = string.Empty;


                                    Command = "EXEC SP_UpdateOrderStatus @p_CustomerID = '" + l_SourceConnector.CustomerID + "', @p_RouteType = '" + RouteTypesEnum.GenerateEDI810ForRepaintRoute + "', @p_OrderId = '" + Convert.ToInt32(item["Id"]) + "'";
                        ...
                        catch (InvalidOperationException)
                        {
                            if (l_ParentNodeNames.Length > 1)
                            {
                                ...
                                while (l_index < l_ParentNodeNames.Length)
                                {
                                    try
                                    {
                                        l_SegmentDef = (MapSegment)(GenerateEDI810ForRepaintRoute.FindMapLoopDef(l_MapLoop, l_ParentNodeNames[l_index])).Content.First(s => s.Name == l_Node.Name);
                                        break;
                                    }
