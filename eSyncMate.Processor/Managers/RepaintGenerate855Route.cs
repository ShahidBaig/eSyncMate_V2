...
{
    public class RepaintGenerate855Route
    {
            ...
            try
            {
                ...
                if (l_DestinationConnector.ConnectivityType == ConnectorTypesEnum.Rest.ToString() && l_dataTable.Rows.Count > 0)
                {
                    ...
                    foreach (DataRow item in l_dataTable.Rows)
                    {
                        ...
                        // If the first deserialization produced a string that itself is JSON, unwrap it
                        string jsonData;
                        ...
                        l_OrderData.DeleteWithType(Convert.ToInt32(item["Id"]), "855-JSON");

                        l_OrderData.Type = "855-JSON";
                        ...
                        OrderStatus855 l_855 = JsonConvert.DeserializeObject<OrderStatus855>(jsonTransformation);

                        Maps_5010.M_855 Map = new Maps_5010.M_855();
                        EdiTrans t = new EdiTrans(Map);
                        int l_HLIndex = 1, l_HL_PIndex = 0;

                        RepaintGenerate855Route.Add855Segments(Map, t, l_855.StartNodes, "L_N9|L_N1", ref l_HLIndex, l_HL_PIndex);

                        foreach (LoopItem l_Item in l_855.Items)
                        {
                            RepaintGenerate855Route.Add855Segments(Map, t, l_Item.Data, "L_PO1|L_ACK", ref l_HLIndex, l_HL_PIndex);
                        }

                        RepaintGenerate855Route.Add855Segments(Map, t, l_855.EndNodes, "L_CTT", ref l_HLIndex, l_HL_PIndex);
                        ...
                        l_Outbound.OrderId = Convert.ToInt32(item["Id"]);
                        ...
                        if (l_Outbound.SaveNew().IsSuccess)
                        {
                            ...
                            l_OData.DeleteWithType(Convert.ToInt32(item["Id"]), "855-eSyncMate");

                            l_OData.Type = "855-eSyncMate";
                            l_OData.Data = w.WriteToString(b);
                            l_OData.CreatedBy = 1;
                            l_OData.CreatedDate = DateTime.Now;
                            l_OData.OrderId = Convert.ToInt32(item["Id"]);
                            ...
                            if ( !string.IsNullOrEmpty(l_OData.Data))
                            {
                                ...
                                Command = "EXEC SP_UpdateOrderStatus @p_CustomerID = '" + l_SourceConnector.CustomerID + "', @p_RouteType = '" + RouteTypesEnum.RepaintGenerate855 + "', @p_OrderId = '" + Convert.ToInt32(item["Id"]) + "'";
                            ...
                            if (l_ParentNodeNames.Length > 1)
                            {
                                ...
                                if (l_SegmentDef == null)
                                {
                                    ...
                                    while (l_index < l_ParentNodeNames.Length)
                                    {
                                        try
                                        {
                                            l_SegmentDef = (MapSegment)(RepaintGenerate855Route.FindMapLoopDef(l_MapLoop, l_ParentNodeNames[l_index])).Content.First(s => s.Name == l_Node.Name);
                                            break;
                                        }
                                        catch (InvalidOperationException)
                                        {
