...
{
    /// <summary>
    /// Knott Warehouse-wise Inventory Upload via Mirakl STO01 API
    ...
    /// </summary>
    public class KnotStockImportRoute
    {
        public static void Execute(IConfiguration config, Routes route)
        {
            ...
            SCSInventoryFeed l_SCSInventoryFeed = new();
            ...
            try
            {
                ...
                route.SaveLog(LogTypeEnum.Info, $"Started executing Knot Stock Import route [{route.Id}]", string.Empty, userNo);
                ...
                if (l_SourceConnector.ConnectivityType == ConnectorTypesEnum.SqlServer.ToString())
                {
                    ...
                    l_SourceConnector.Command = l_SourceConnector.Command.Replace("@ROUTETYPEID@", Convert.ToString(RouteTypesEnum.KnotWHSWInventoryUpload));
                ...
                if (l_DestinationConnector.ConnectivityType == ConnectorTypesEnum.Rest.ToString() && l_data.Rows.Count > 0)
                {
                    ...
                    l_InventoryBatchWise.RouteType = RouteTypesEnum.KnotWHSWInventoryUpload.ToString();
                    ...
                    {
                        var itemsThread = new ProcessKnotStockImportChunkThread(table, route, feed, l_DestinationConnector, l_SourceConnector, userNo, l_InventoryBatchWise.BatchID, ShipNodedataTable);
                ...
                route.SaveLog(LogTypeEnum.Info, $"Completed Knot Stock Import route [{route.Id}]", string.Empty, userNo);
            }
            catch (Exception ex)
            {
                l_InventoryBatchWise.FinishDate = DateTime.Now;
                ...
                l_SCSInventoryFeed.UpdateInventoryBatchWise(l_InventoryBatchWise);
                route.SaveLog(LogTypeEnum.Exception, $"Error executing Knot Stock Import route [{route.Id}]", ex.ToString(), userNo);
            }
            finally
            {
                l_data.Dispose();
    ...
    public class ProcessKnotStockImportChunkThread
    {
        ...
        public ProcessKnotStockImportChunkThread(DataTable data, Routes route, SCSInventoryFeed feed, ConnectorDataModel destinationConnector, ConnectorDataModel sourceConnector, int userNo, string batchID, DataTable shipNodeData)
        {
            this.data = data;
        ...
        public async Task ProcessItems()
        {
            ...
            try
            {
                ...
                this.feed.BulkNewInsertData(this.sourceConnector.ConnectionString, "SCSInventoryFeedData", sentTable);
                {
                    ...
                    {
                        this.route.SaveLog(LogTypeEnum.Error, "KnotStockImportRoute STO01 failed.", uploadBody, this.userNo);
                        return;
                    }
                    ...
                    {
                        this.route.SaveLog(LogTypeEnum.Error, "KnotStockImportRoute STO01 did not return import_id.", uploadBody, this.userNo);
                        return;
                    }

                    this.feed.UpdateSCSKnotFeedData(this.batchID, importId, l_Guid);
        ...
        private string BuildStockImportCsv(DataTable inventoryData, SCSInventoryFeed feed, string connectionString, string batchID, DataTable shipNodeData, string p_Guid, out DataTable sentTable)
        {
            ...
            try
            {
                ...
                DataTable bulkSCSKnotFeedData = CreateBulkInsertSCSKnotFeedData();

                string header = feed.GetKnotStockImportHeader();
                ...
                foreach (DataRow row in inventoryData.Rows)
                {
                    ...
                    string offerSku = row["CustomerItemCode"]?.ToString() ?? "";
                    ...
                    DataRow knotFeedRow = bulkSCSKnotFeedData.NewRow();
                    knotFeedRow["BatchID"] = batchID;
                    knotFeedRow["ItemID"] = itemId;
                    knotFeedRow["CustomerID"] = customerId;
                    knotFeedRow["ImportId"] = p_Guid;
                    knotFeedRow["Data"] = perItemJson;
                    bulkSCSKnotFeedData.Rows.Add(knotFeedRow);
                }

                feed.BulkKnotFeedData(connectionString, "SCSKnotFeedData", bulkSCSKnotFeedData);
        ...
        public static DataTable CreateBulkInsertSCSKnotFeedData()
        {
            DataTable bulkInsertTable = new();
            bulkInsertTable.Columns.Add("BatchID", typeof(string));
            bulkInsertTable.Columns.Add("ItemID", typeof(string));
        ...
        private static TimeSpan GetRetryDelay(int attempt)
        {
            ...
            int jitterMs;
            lock (RetryRandom) { jitterMs = RetryRandom.Next(0, 1000); }
        ...
        private static async Task<HttpResponseMessage> PostStockImportCsv(ConnectorDataModel connector, string csv)
        {
            ...
            throw new HttpRequestException("Unexpected failure sending Knot stock import request.");
        }
