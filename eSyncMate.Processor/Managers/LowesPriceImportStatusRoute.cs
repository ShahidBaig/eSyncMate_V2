...
{
    /// <summary>
    /// Lowes Price Import Status Check via Mirakl PRI02 API
    ///
    /// Flow:
    ...
    ///   3. Log results
    /// </summary>
    public class LowesPriceImportStatusRoute
    {
        public static void Execute(IConfiguration config, Routes route)
        {
            ...
            try
            {
                ...
                route.SaveLog(LogTypeEnum.Info, $"Started executing Lowes Price Import Status route [{route.Id}]", string.Empty, userNo);
                ...
                l_SourceConnector.Command = l_SourceConnector.Command.Replace("@ROUTETYPEID@", Convert.ToString(RouteTypesEnum.LowesPriceImportStatus));
                ...
                foreach (DataRow row in dtPending.Rows)
                {
                    ...
                    try
                    {
                        ...
                        // Parse response
                        var response = JsonConvert.DeserializeObject<LowesPriceImportStatusResponseModel>(statusResult);
                        ...
                        // Also update InventoryBatchWise if COMPLETE or FAILED
            ...
            {
                route.SaveLog(LogTypeEnum.Exception, $"Error executing Lowes Price Import Status route [{route.Id}]", ex.ToString(), userNo);
            }
        }

        /// <summary>
        /// Calls Mirakl PRI02: GET /api/offers/pricing/imports?import_id={importId}
