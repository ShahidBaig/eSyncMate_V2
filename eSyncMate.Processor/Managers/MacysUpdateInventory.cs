...
{
    public class MacysUpdateInventory
    {
        public static void Execute(IConfiguration config, Routes route)
        {
            ...
            try
            {
                ...
                if (l_DestinationConnector.ConnectivityType == ConnectorTypesEnum.Rest.ToString() && l_data.Rows.Count > 0)
                {
                    ...
                    l_InventoryBatchWise.RouteType = RouteTypesEnum.MacysInventoryUpload.ToString();
                    ...
                    // Log UPLOAD snapshot — captures inventory state sent to Macy's
    ...
    public class ProcessMacysItemsThread
    {
        ...
        public ProcessMacysItemsThread(DataTable data, Routes route, SCSInventoryFeed feed, ConnectorDataModel destinationConnector,
