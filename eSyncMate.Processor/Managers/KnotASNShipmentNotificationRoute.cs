...
{
    public class KnotASNShipmentNotificationRoute
    {
            ...
            try
            {
                ...
                if (l_DestinationConnector.ConnectivityType == ConnectorTypesEnum.Rest.ToString() && l_dataTable.Rows.Count > 0)
                {
                    ...
                    foreach (DataRow l_Row in l_Orders.Rows)
                    {
                        ...
                        KnotAsnRequestModel l_KnotAsnRequestModel = new KnotAsnRequestModel();
                        KnotShipment l_KnotShipment = new KnotShipment();
                        KnotShipment_Lines KnotShipment_Lines = new KnotShipment_Lines();
                        ...
                        {
                            l_KnotShipment = new KnotShipment();
                            KnotShipment_Lines = new KnotShipment_Lines();

                            l_KnotShipment.order_id = l_VRow["OrderNumber"].ToString();
                            l_KnotShipment.invoice_reference = l_VRow["TrackingNo"].ToString();
                            l_KnotShipment.tracking.carrier_code = l_VRow["LevelOfService"].ToString();
                        ...
                        l_Row["Data"] = JsonConvert.SerializeObject(l_KnotAsnRequestModel);
                    }
        ...
        // Resolves this customer's Knot ASN route and re-sends ONLY the given order. Its own copy of
        ...
        public static string ExecuteSingle(IConfiguration config, int orderId, string customerName)
        {
            ...
            if (!route.GetObject("TypeId", (int)RouteTypesEnum.KnotASNShipmentNotification, "CustomerName", customerName).IsSuccess)
            {
