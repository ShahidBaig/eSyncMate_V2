...
{
    public class MacysGetOrderRoute
    {
        public static void Execute(IConfiguration config, Routes route)
        {
            ...
            try
            {
                ...
                if (l_SourceConnector.ConnectivityType == ConnectorTypesEnum.Rest.ToString())
                {
                    ...
                    //string orderStatuses = "WAITING_ACCEPTANCE,SHIPPING";     //ACKNOWLEDGED => SHIPPING
                    //l_SourceConnector.Url = $"{l_SourceConnector.BaseUrl}/api/orders?start_date={formattedEndDate}&order_state_codes={orderStatuses}";
        ...
        private static MacysOrder GetOrdersData(Routes route, string orderNumber)
        {
            RestClient client;
            RestRequest request;
            ...
            return JsonConvert.DeserializeObject<MacysOrder>(response.Content);
        }
