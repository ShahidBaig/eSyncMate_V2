...
{
    public class LowesCancellationRoute
    {
        public static void Execute(IConfiguration config, Routes route)
        {
            ...
            LowesCancelOrderInputModel l_InputCancellationLinesModel = new LowesCancelOrderInputModel();
            LowesCancelation l_Order_Line_Cancelation = new LowesCancelation();
            try
            {
                ...
                if (l_DestinationConnector.ConnectivityType == ConnectorTypesEnum.Rest.ToString() && l_dataTable.Rows.Count > 0)
                {
                    ...
                    {
                        l_InputCancellationLinesModel = new LowesCancelOrderInputModel();

                        if (Convert.ToInt32(l_Row["CancelQty"]) > 0)
                        {
                            l_Order_Line_Cancelation = new LowesCancelation();
                        ...
                        if (sourceResponse.StatusCode == System.Net.HttpStatusCode.OK || sourceResponse.StatusCode == System.Net.HttpStatusCode.Created)
                        {
                            ...
                            route.SaveLog(LogTypeEnum.Debug, $"LowesCancelOrder processed for order [{l_Row["Id"]}].", string.Empty, userNo);
