...
{
    public class BulkUploadPricesRoute
    {
        public static void Execute(IConfiguration config, ILogger logger, Routes route)
        {
            ...
            try
            {
                ...
                if (l_SourceConnector.ConnectivityType == ConnectorTypesEnum.SqlServer.ToString())
                {
                    ...
                    {
                        l_ProductUploadPrices.GetProductID($"Status = 'NEW' AND PromoStartDate <= GETDATE() AND PromoEndDate > GETDATE() AND CustomerID = '{l_SourceConnector.CustomerID}'", ref l_Sourcedata);
                    }
                ...
                if (l_DestinationConnector.ConnectivityType == ConnectorTypesEnum.Rest.ToString() && l_Sourcedata.Rows.Count > 0)
                {
                    ...
                    foreach (DataRow l_Row in l_Sourcedata.Rows)
                    {
                        ...
                        //{
                        //    route.SaveLog(LogTypeEnum.Debug, $"GetProductPrice processed for [{l_Row["ProductID"]}].", string.Empty, userNo);
                        //}
                        //else
                        //{
                        //    route.SaveLog(LogTypeEnum.Debug, $"GetProductPrice not processed for [{l_Row["ProductID"]}].", string.Empty, userNo);
                        //    continue;
                        //}
                        ...
                        //var data = JsonConvert.DeserializeAnonymousType(sourceResponse.Content, l_oldPrices);
                        //if (data != null)
                        //{
                        //    l_ProductUploadPrices.UpdateOldPrices(Convert.ToString(l_Row["ItemId"]), data.list_price, data.offer_price);
                        //}

                        var l_newPrices = new
                        {
                            list_price = l_Row["ListPrice"],
                            offer_price = l_Row["OffPrice"],
                        };
                        ...
                        if (sourceResponse.StatusCode == System.Net.HttpStatusCode.OK)
                        {
                            ...
                            l_ProductUploadPrices.UpdateStatus(Convert.ToString(l_Row["ItemId"]), "SYNCED");
                        }
                        else
                        {
