...
{
    public class ASNShipmentNotificationRoute
    {
            ...
            try
            {
                ...
                if (l_DestinationConnector.ConnectivityType == ConnectorTypesEnum.Rest.ToString() && l_dataTable.Rows.Count > 0)
                {
                    ...
                    foreach (DataRow l_Row in l_Orders.Rows)
                    {
                        ...
                        l_DestinationConnector.Url = l_DestinationConnector.BaseUrl + l_Row["OrderNumber"] + "/bulk_fulfillments_create";
        ...
        // Resolves this customer's ASN route and re-sends ONLY the given order. Its own copy of the
        // send logic â€” the batch Execute above is never touched. Source SP is constrained with
        ...
        public static string ExecuteSingle(IConfiguration config, int orderId, string customerName)
        {
            ...
            if (!route.GetObject("TypeId", (int)RouteTypesEnum.ASNShipmentNotification, "CustomerName", customerName).IsSuccess)
            {
