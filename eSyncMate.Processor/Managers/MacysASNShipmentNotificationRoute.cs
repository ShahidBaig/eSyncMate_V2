...
{
    public class MacysASNShipmentNotificationRoute
    {
            ...
            try
            {
                ...
                if (l_DestinationConnector.ConnectivityType == ConnectorTypesEnum.Rest.ToString() && l_dataTable.Rows.Count > 0)
                {
                    ...
                    foreach (DataRow l_Row in l_Orders.Rows)
                    {
                        ...
                        MacysAsnRequestModel l_MacysAsnRequestModel = new MacysAsnRequestModel();
                        MacysShipment l_MacysShipment = new MacysShipment();
                        MacysShipment_Lines MacysShipment_Lines = new MacysShipment_Lines();
                        ...
                        {
                            l_MacysShipment = new MacysShipment();
                            MacysShipment_Lines = new MacysShipment_Lines();

                            l_MacysShipment.order_id = l_VRow["OrderNumber"].ToString();
                            l_MacysShipment.invoice_reference = l_VRow["TrackingNo"].ToString();
                            l_MacysShipment.tracking.carrier_code = l_VRow["LevelOfService"].ToString();
                            l_MacysShipment.tracking.carrier_name = l_VRow["LevelOfService"].ToString();
                            l_MacysShipment.tracking.tracking_number = l_VRow["TrackingNo"].ToString();

                            MacysShipment_Lines.offer_sku = l_VRow["ItemID"].ToString();
                            MacysShipment_Lines.quantity = Convert.ToInt32(l_VRow["LineQty"]);
                            l_MacysShipment.shipment_lines.Add(MacysShipment_Lines);

                            l_MacysAsnRequestModel.shipments.Add(l_MacysShipment);
                        ...
                        l_Row["Data"] = JsonConvert.SerializeObject(l_MacysAsnRequestModel);
                    }
        ...
        // Resolves this customer's Macys ASN route and re-sends ONLY the given order. Its own copy of
        ...
        public static string ExecuteSingle(IConfiguration config, int orderId, string customerName)
        {
            ...
            if (!route.GetObject("TypeId", (int)RouteTypesEnum.MacysASNShipmentNotification, "CustomerName", customerName).IsSuccess)
            {
