...
{
    public class SCSUpdateInventory
    {
        public static void Execute(IConfiguration config, ILogger logger, Routes route)
        {
            ...
            try
            {
                ...
                if (l_SourceConnector.ConnectivityType == ConnectorTypesEnum.SqlServer.ToString())
                {
                    ...
                    l_SourceConnector.Command = l_SourceConnector.Command.Replace("@ROUTETYPEID@", Convert.ToString(RouteTypesEnum.SCSUpdateInventory));
                ...
                if (l_DestinationConnector.ConnectivityType == ConnectorTypesEnum.Rest.ToString() && l_data.Rows.Count > 0)
                {
                    ...
                    l_InventoryBatchWise.RouteType = RouteTypesEnum.SCSUpdateInventory.ToString();
                    ...
                    {
                        ProcessItemsThread itemsThread = new ProcessItemsThread(l_data, route, feed, l_DestinationConnector, l_SourceConnector, userNo, l_InventoryBatchWise.BatchID);
                    ...
                    else
                    {
                        ...
                        {
                            ProcessItemsThread itemsThread = new ProcessItemsThread(tables[i], route, feed, l_DestinationConnector, l_SourceConnector, userNo, l_InventoryBatchWise.BatchID);
    ...
    public class ProcessItemsThread
    {
        ...
        public ProcessItemsThread(DataTable data, Routes route, SCSInventoryFeed feed, ConnectorDataModel destinationConnector,
