...
{
    public class MichealUpdatePrice
    {
        public static void Execute(IConfiguration config, ILogger logger, Routes route)
        {
            ...
            try
            {
                ...
                if (l_SourceConnector.ConnectivityType == ConnectorTypesEnum.SqlServer.ToString())
                {
                    ...
                    l_SourceConnector.Command = l_SourceConnector.Command.Replace("@ROUTETYPEID@", Convert.ToString(RouteTypesEnum.MichealBulkItemPrices));
                ...
                if (l_DestinationConnector.ConnectivityType == ConnectorTypesEnum.Rest.ToString() && l_data.Rows.Count > 0)
                {
                    ...
                    l_InventoryBatchWise.RouteType = RouteTypesEnum.MichealBulkItemPrices.ToString();
                    ...
                    {
                        ProcessMichleItemsThread itemsThread = new ProcessMichleItemsThread(l_data, route, feed, l_DestinationConnector, l_SourceConnector, userNo, l_InventoryBatchWise.BatchID);
                    ...
                    else
                    {
                        ...
                        {
                            ProcessMichleItemsThread itemsThread = new ProcessMichleItemsThread(tables[i], route, feed, l_DestinationConnector, l_SourceConnector, userNo, l_InventoryBatchWise.BatchID);
    ...
    public class ProcessMichleItemsThread
    {
        ...
        public ProcessMichleItemsThread(DataTable data, Routes route, SCSInventoryFeed feed, ConnectorDataModel destinationConnector,
        ...
        public void ProcessItem(DataRow row)
        {
            ...
            string customerId = row["CustomerId"].ToString();
            string itemId = row["ItemId"].ToString();
            List<MichealPriceUploadRequestModel> l_MichealPriceUploadRequestModel = new List<MichealPriceUploadRequestModel>();
            ...
            try
            {
                ...
                string Body = JsonConvert.SerializeObject(l_MichealPriceUploadRequestModel);
                ...
                this.destinationConnector.Url = this.destinationConnector.BaseUrl + "/listing/price/publish-by-seller-sku-number";
                ...
                if (sourceResponse.StatusCode == System.Net.HttpStatusCode.OK || sourceResponse.StatusCode == System.Net.HttpStatusCode.Created)
                {
                    ...
                    this.route.SaveLog(LogTypeEnum.Debug, $"MichealUpdatePrice updated for item [{row["ProductId"]}].", sourceResponse.Content, this.userNo);
                }
                else
                {
            ...
            {
                this.route.SaveLog(LogTypeEnum.Exception, $"Unable to update MichealUpdatePrice for item [{row["ProductId"]}].", ex.ToString(), this.userNo);
            }
        }
    }
}
