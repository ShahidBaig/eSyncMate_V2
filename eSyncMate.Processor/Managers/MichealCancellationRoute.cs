...
using static eSyncMate.Processor.Models.MichealCancelOrderInputModel;

namespace eSyncMate.Processor.Managers
{
    public class MichealCancellationRoute
    {
        public static void Execute(IConfiguration config, ILogger logger, Routes route)
        {
            ...
            MichealCancelOrderInputModel l_InputCancellationLinesModel = new MichealCancelOrderInputModel();
            MichealCancelorderline l_Order_Line_Cancelation = new MichealCancelorderline();
            try
            {
                ...
                if (l_DestinationConnector.ConnectivityType == ConnectorTypesEnum.Rest.ToString() && l_dataTable.Rows.Count > 0)
                {
                    ...
                    {
                        l_InputCancellationLinesModel = new MichealCancelOrderInputModel();

                        if (Convert.ToInt32(l_Row["CancelQty"]) > 0)
                        {
                            l_Order_Line_Cancelation = new MichealCancelorderline();

                            l_InputCancellationLinesModel.cancelReason = "";
                        ...
                        l_DestinationConnector.Url = l_DestinationConnector.BaseUrl + "/order/cancel-order";
                        ...
                        if (sourceResponse.StatusCode == System.Net.HttpStatusCode.OK || sourceResponse.StatusCode == System.Net.HttpStatusCode.Created)
                        {
                            ...
                            route.SaveLog(LogTypeEnum.Debug, $"MichealCancelOrder processed for order [{l_Row["Id"]}].", string.Empty, userNo);
