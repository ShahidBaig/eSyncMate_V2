...
{
    public class LowesBulkItemPricesRoute
    {
        public static void Execute(IConfiguration config, Routes route)
        {
            ...
            try
            {
                ...
                if (l_SourceConnector.ConnectivityType == ConnectorTypesEnum.SqlServer.ToString())
                {
                    ...
                    l_SourceConnector.Command = l_SourceConnector.Command.Replace("@ROUTETYPEID@", Convert.ToString(RouteTypesEnum.LowesBulkItemPrices));
                ...
                if (l_DestinationConnector.ConnectivityType == ConnectorTypesEnum.Rest.ToString() && l_data.Rows.Count > 0)
                {
                    ...
                    if (sourceResponse.StatusCode == System.Net.HttpStatusCode.Created)
                    {
                        foreach (DataRow row in l_data.Rows)
                        {
                            route.SaveLog(LogTypeEnum.Debug, $"Lowes Bulk ItemPrices updated for item [{row["id"]}].", string.Empty, userNo);
    ...
    public class ProcessLoweBulkItemPricesThread
    {
        ...
        public ProcessLoweBulkItemPricesThread(DataTable data, Routes route, ConnectorDataModel destinationConnector,
