...
using static eSyncMate.Processor.Models.LowesCancelOrderInputModel;
using static eSyncMate.Processor.Models.KnotCancelOrderInputModel;

namespace eSyncMate.Processor.Managers
{
    public class KnotCancellationRoute
    {
        public static void Execute(IConfiguration config, Routes route)
        {
            ...
            KnotCancelOrderInputModel l_InputCancellationLinesModel = new KnotCancelOrderInputModel();
            KnotCancelation l_Order_Line_Cancelation = new KnotCancelation();
            try
            {
                ...
                if (l_DestinationConnector.ConnectivityType == ConnectorTypesEnum.Rest.ToString() && l_dataTable.Rows.Count > 0)
                {
                    ...
                    {
                        l_InputCancellationLinesModel = new KnotCancelOrderInputModel();

                        if (Convert.ToInt32(l_Row["CancelQty"]) > 0)
                        {
                            l_Order_Line_Cancelation = new KnotCancelation();

                            l_Order_Line_Cancelation.amount = Convert.ToDecimal(l_Row["UnitPrice"]);
                            l_Order_Line_Cancelation.currency_iso_code = "USD";
                            l_Order_Line_Cancelation.order_line_id = l_Row["order_line_id"].ToString();
                            l_Order_Line_Cancelation.quantity = Convert.ToInt32(l_Row["CancelQty"]);
                            l_Order_Line_Cancelation.reason_code = "1";
                            l_Order_Line_Cancelation.shipping_amount = 0;

                            l_InputCancellationLinesModel.cancelations.Add(l_Order_Line_Cancelation);
                        }

                        Body = JsonConvert.SerializeObject(l_InputCancellationLinesModel);
                        route.SaveData("JSONCANLN-SNT", 0, Body, userNo);

                        l_DestinationConnector.Url = l_DestinationConnector.BaseUrl + "/api/orders/cancel";
                        ...
                        if (sourceResponse.StatusCode == System.Net.HttpStatusCode.OK || sourceResponse.StatusCode == System.Net.HttpStatusCode.Created)
                        {
                            ...
                            route.SaveLog(LogTypeEnum.Debug, $"KnotCancelOrder processed for order [{l_Row["Id"]}].", string.Empty, userNo);
