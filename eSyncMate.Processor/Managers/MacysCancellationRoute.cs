...
using static eSyncMate.Processor.Models.MacysCancelOrderInputModel;

namespace eSyncMate.Processor.Managers
{
    public class MacysCancellationRoute
    {
        public static void Execute(IConfiguration config, ILogger logger, Routes route)
        {
            ...
            MacysCancelOrderInputModel l_InputCancellationLinesModel = new MacysCancelOrderInputModel();
            Cancelation l_Order_Line_Cancelation = new Cancelation();
            try
            {
                ...
                if (l_DestinationConnector.ConnectivityType == ConnectorTypesEnum.Rest.ToString() && l_dataTable.Rows.Count > 0)
                {
                    ...
                    {
                        l_InputCancellationLinesModel = new MacysCancelOrderInputModel();

                        if (Convert.ToInt32(l_Row["CancelQty"]) > 0)
                        {
                            l_Order_Line_Cancelation = new Cancelation();
                            ...
                            l_Order_Line_Cancelation.reason_code = "CANCELATION_03";
                        ...
                        if (sourceResponse.StatusCode == System.Net.HttpStatusCode.OK || sourceResponse.StatusCode == System.Net.HttpStatusCode.Created)
                        {
                            ...
                            route.SaveLog(LogTypeEnum.Debug, $"MacysCancelOrder processed for order [{l_Row["Id"]}].", string.Empty, userNo);
