...
namespace eSyncMate.Processor.Controllers
{
    ...
    public class PartnerGroupsController : ControllerBase
    {
        ...
        public PartnerGroupsController(ILogger<PartnerGroupsController> logger)
        {
            _logger = logger;
        }
        ...
        public async Task<GetPartnerGroupsResponseModel> GetPartnerGroups([FromQuery] PartnerGroupSearchModel searchModel)
        {
            ...
            try
            {
                ...
                int totalCount = 0;
                l_PartnerGroups.GetViewListPaged(l_Criteria, string.Empty, ref l_Data, "Id DESC", searchModel.PageNumber, searchModel.PageSize, out totalCount);

                this._logger.LogDebug($"[{l_Me.ReflectedType.Name}.{l_Me.Name}] - PartnerGroups searched {{{l_Data.Rows.Count}}} of {totalCount} total.");
