/* ------------------------------------------------------------------
   75 — SP_OrdersData WITHOUT the ShipCodes changes (original hardcoded mapping)

   This is SP_OrdersData with the per-customer LevelOfService / ShippingMethod
   mapping as hardcoded CASE statements — i.e. NO dependency on fn_ResolveShipCode
   or the ShipCodeMappings table. Use this when the ShipCodes Mapping interface is
   hidden / not deployed. (Script 68 is the ShipCodes/fn-based version — do not run
   that one if you use this.)

   SINGLE-ORDER (Re-Transmit ASN): every customer-wise ASN branch now honours
   @p_OrderId. When @p_OrderId = 0 the batch behaviour is UNCHANGED. When @p_OrderId
   <> 0 the branch returns just that order, bypasses the O.Status = SYNCED filter and
   ALSO includes lines already sent (D.Status = 'ASNSNT'), so a SHIPPED order can be
   re-selected for re-transmit.

   Verify against your live SP before deploying if a newer client version exists.
   ------------------------------------------------------------------ */

CREATE OR ALTER PROCEDURE [dbo].[SP_OrdersData]
    ...
    BEGIN TRY
        ...
        BEGIN
            ...
            BEGIN
                ...
                WHERE C.ERPCustomerID = @l_CustomerID AND (O.Status = @l_OrderStatus OR @l_OrderId <> 0) AND ISNULL(D.TrackingNo, '') <> ''
                    AND (ISNULL(D.[Status], '') = 'ASNRVD' OR (@l_OrderId <> 0 AND ISNULL(D.[Status], '') = 'ASNSNT'))
                    AND (@l_OrderId = 0 OR O.Id = @l_OrderId)
            ...
            BEGIN
                ...
                FROM Orders O WITH (NOLOCK)
                    ...
                    INNER JOIN OrderDetail D WITH (NOLOCK) ON O.Id = D.OrderId
                WHERE C.ERPCustomerID = @l_CustomerID AND (O.Status = @l_OrderStatus OR @l_OrderId <> 0) AND ISNULL(D.TrackingNo, '') <> ''
                    AND (ISNULL(D.[Status], '') = 'ASNRVD' OR (@l_OrderId <> 0 AND ISNULL(D.[Status], '') = 'ASNSNT')) --AND O.Id = 234033
                    AND (@l_OrderId = 0 OR O.Id = @l_OrderId)
                GROUP BY O.Id, O.OrderNumber, O.ExternalId, D.[LineNo], D.TrackingNo, D.ShippedDate
            END
            ELSE IF @l_CustomerID IN ('AMA1000')
            BEGIN
                    SELECT O.Id, O.OrderNumber, O.ExternalId, D.[LineNo], D.TrackingNo, D.ShippedDate, SUM(D.LineQty) LineQty,
                    MAX(CASE WHEN D.ShippingMethod = 'FEDEX HOME DELIVERY' OR D.ShippingMethod = 'FEDEX GROUND' THEN 'FedEx' WHEN D.ShippingMethod LIKE 'UPS %' THEN 'UPSG' ELSE 'FedEx' END) LevelOfService,
                    MAX(CASE WHEN D.ShippingMethod = 'FEDEX HOME DELIVERY' THEN 'FedExGroundHomeDelivery' WHEN D.ShippingMethod = 'FEDEX GROUND' THEN 'FedEx Ground' ELSE 'FedEx Ground' END ) ShippingMethod,
                ...
                GROUP BY O.Id, O.OrderNumber, O.ExternalId, D.[LineNo], D.TrackingNo, D.ShippedDate
            END
            ELSE IF @l_CustomerID IN ('MOR4778')
            BEGIN
                    SELECT O.Id, O.OrderNumber, O.ExternalId, D.[LineNo], D.TrackingNo, D.ShippedDate, SUM(D.LineQty) LineQty,
                    MAX(CASE WHEN D.ShippingMethod = 'FEDEX HOME DELIVERY' OR D.ShippingMethod = 'FEDEX GROUND' THEN 'FedEx' WHEN D.ShippingMethod LIKE 'UPS %' THEN 'UPSG' ELSE 'FedEx' END) LevelOfService,
                    MAX(CASE WHEN D.ShippingMethod = 'FEDEX HOME DELIVERY' THEN 'FedExGroundHomeDelivery' WHEN D.ShippingMethod = 'FEDEX GROUND' THEN 'FedEx Ground' ELSE 'FedEx Ground' END ) ShippingMethod,
